...
CANDIDATE_ID="${1:-LEA-197}"
BRANCH="${2:-leonpcsn/lea-197-implementar-application-engine-e-panel-runtime-comum}"
REPO="${TRIVIEW_REPO:-leon337/triview-workspace-linux}"
SAFE_ID="$(printf '%s' "$CANDIDATE_ID" | tr '[:upper:]' '[:lower:]' | tr -cd 'a-z0-9._-')"
[[ -n "$SAFE_ID" ]] || { printf 'Identificador de candidato inválido.\n' >&2; exit 2; }
...
[[ -d "$tmp_dir/extracted/src/triview_workspace" ]] || fail "Pacote inválido: código ausente."
...
log "Candidato instalado sem alterar a versão principal."
log "Atalho: TriView Workspace — $CANDIDATE_ID"
log "Código: $current_link"
log "Dados isolados: $DATA_ROOT"
